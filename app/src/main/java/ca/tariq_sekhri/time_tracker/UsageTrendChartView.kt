...
class UsageTrendChartView(context: Context, private val values: List<Long>, private val axisLabels: List<String>, private val barMode: Boolean) : View(context) {
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (barMode) drawBars(canvas,left,top,w,bottom,maxValue) else drawLine(canvas,left,top,w,bottom,maxValue)
    }
    ...
    private fun drawBars(canvas:Canvas,left:Float,top:Float,w:Float,bottom:Float,maxValue:Float) {
        val usable=w-left-dp(12f); val cell=usable/values.size.coerceAtLeast(1)
        values.forEachIndexed { i,value ->
            val barHeight=(bottom-top)*value/maxValue
            paint.style=Paint.Style.FILL; paint.color=Color.rgb(190,157,255)
            canvas.drawRoundRect(left+cell*i+cell*.13f,bottom-barHeight,left+cell*(i+1)-cell*.13f,bottom,dp(3f),dp(3f),paint)
        }
        labels(canvas,axisLabels,left,w,bottom)
    }
